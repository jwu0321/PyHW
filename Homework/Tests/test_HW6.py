...
def test_digit_sum():
    ...
    if (s := digit_sum(0)) != 0:
        errors.append(f'Expected: 0, Actual {s}')
    if (s := digit_sum(1)) != 1:
...
def test_hanoi():
    ...
    for i in range(15):
        if (s := hanoi(i)) != 2**i - 1:
            errors.append(f'Expected: {2**i - 1}, Actual: {s}')
...
def test_subset_sum():
    ...
    if b := subset_sum([], 0):
        errors.append(f'Expected: False, Actual: {b}')
    if b := subset_sum([], 1):
        errors.append(f'Expected: False, Actual: {b}')
    if not (b := subset_sum([1], 1)):
        errors.append(f'Expected: True, Actual: {b}')
    ...
    if not (b := subset_sum([2, 1, 5, 17, 7, -13, -19, 11], -27)):
        errors.append(f'Expected: True, Actual: {b}')
    if b := subset_sum([2, 1, 5, 17, 7, -13, -19, 11], 43):
        errors.append(f'Expected: False, Actual: {b}')
...
def test_permute():
    ...
    for array in lists:
        ...
        if missing:
            errors.append(f'Missing permutations: {", ".join([str(m) for m in missing])} for {array}')
